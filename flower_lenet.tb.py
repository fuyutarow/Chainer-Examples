...
with SummaryWriter('runs/'+datetime.now().strftime('%B%d  %H:%M:%S')) as writer:
    writer.add_graph([res])
    writer.add_all_variable_images([res], pattern='.*LeNet.*')
    writer.add_all_parameter_histograms([res], pattern='.*LeNet.*')
